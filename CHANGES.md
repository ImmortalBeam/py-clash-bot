...
- 2026-09-16 daytime watch (73 scored matches): the first threat-tier build went 9-14 (single units on the tower were ignored); adding tower-band and incoming-band detection went 15-18; adding once-only incoming triggers, a correct cooldown memory and no cheap cards into large pushes went 9-7. Rules added: enemy bars in the tower band (y>=330) are always at least a medium threat; a group above the river on their side triggers one defend when it appears; no chip play before 20 s; large threats take building, tank, support or spell only.
